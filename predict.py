...
def predict(model, df, start, end):

    def create_feature(df, start, end):
        # create features from the selected machine
        pressure = df.loc[start: end, 'pressure']
        start = df[df['datetime']==pd.to_datetime(start)].index[0]
        end = df[df['datetime']==pd.to_datetime(end)].index[0]
    ...
    X_test, _, _ = create_feature(df, start, end)
...
def failure(model, df, start, end):
    """
    ...
    `   The starting time of the prediction
        ...
        The ending time of the prediction
    ...
    predictions = predict(model, df, start, end)
